...
export default function SearchBar({ lang }) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 relative z-[1000] -mt-10 md:-mt-14" ref={dropdownRef}>
      ...
      <form 
        ...
        className={`bg-white/95 backdrop-blur-xl p-2 md:p-3 rounded-[3rem] transition-all duration-500 flex items-center overflow-visible shadow-2xl ${isFocused ? 'ring-2 ring-[#C02026]' : ''}`}
      >
        <div className={`flex items-center justify-center ps-4 md:ps-8 transition-colors ${isFocused ? 'text-[#C02026]' : 'text-slate-400'}`}>
        ...
        <input
          ...
          className="w-full bg-transparent border-none outline-none ring-0 focus:ring-0 px-6 py-4 md:py-6 text-lg md:text-xl text-slate-900 font-extrabold placeholder:text-slate-400 italic tracking-tight leading-relaxed"
        />

        {query && (
          <button type="button" onClick={() => setQuery('')} className="p-3 text-slate-400 hover:text-red-500 transition-colors">
            <X size={22} />
          </button>
        )}

        <button type="submit" className="hidden md:flex bg-[#C02026] text-white px-12 py-5 rounded-[2rem] font-black items-center gap-3 hover:bg-slate-950 transition-all shadow-xl uppercase italic tracking-widest active:scale-95 shrink-0">
          {isAr ? 'بحث' : 'Find'}
        </button>
      </form>

      {showDropdown && (
        <div className="absolute top-full inset-x-4 mt-6 bg-white/98 backdrop-blur-3xl rounded-[3.5rem] shadow-premium border border-slate-50 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-500 text-start">
          <div className="max-h-[60vh] overflow-y-auto p-8 md:p-12 space-y-12 custom-scrollbar">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 border-t border-slate-50 pt-10 text-start">
                ...
                <div className="space-y-6">
                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] italic flex items-center gap-3 ps-2">
                    <Building2 size={14} /> {isAr ? 'المطورون' : 'Titans'}
                  </h4>
                  <div className="flex flex-wrap gap-2">
          ...
          <div className="p-6 bg-slate-950 text-center">
             <Link href={`/${lang}/projects/`} className="text-[10px] font-black text-white/50 hover:text-white uppercase tracking-[0.5em] transition-colors">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .shadow-premium { box-shadow: 0 60px 120px -20px rgba(0,0,0,0.2); }
