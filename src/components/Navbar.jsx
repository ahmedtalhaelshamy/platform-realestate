...
export default function Navbar({ lang }) {
  ...
  // 1. مراقبة التمرير لتحسين تجربة المستخدم (Glassmorphism Effect)
  useEffect(() => {
    const handleScroll = () => {
  ...
  // 2. جلب المناطق بذكاء مع تفعيل الـ Cache لعام 2026
  useEffect(() => {
  ...
  // توحيد روابط اللغة (SEO Standard)
  ...
  return (
    <>
      <nav 
        className={`fixed w-full z-[9999] transition-all duration-500 ease-in-out font-sans ${navBackground}`} 
        dir={isAr ? 'rtl' : 'ltr'}
      >
            ...
            <div className="hidden lg:flex items-center justify-center flex-1 gap-10">
              ...
              <div className="relative group h-10 flex items-center"> 
                ...
                <div className="absolute top-full pt-6 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 w-[300px] ltr:left-1/2 ltr:-translate-x-1/2 rtl:right-1/2 rtl:translate-x-1/2">
                  <div className="bg-white rounded-[2rem] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.2)] border border-slate-50 overflow-hidden p-3">
                    <div className="grid gap-1">
                        {sanityLocations.slice(0, 8).map((loc) => (
                          <Link key={loc.slug} href={`/${currentLang}/locations/${loc.slug}/`} className="flex items-center gap-4 px-5 py-3.5 text-xs font-black text-slate-700 hover:bg-slate-50 hover:text-[#C02026] transition-all rounded-2xl group/item">
                            <MapPin size={16} className="text-slate-300 group-hover/item:text-[#C02026] transition-colors" />
                    ...
                    <div className="p-2 mt-2 border-t border-slate-50">
                      <Link href={`/${currentLang}/locations/`} className="flex items-center justify-center w-full py-4 bg-slate-950 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-[#C02026] transition-all shadow-xl">
            ...
            <div className="hidden lg:flex items-center justify-end gap-5 shrink-0">
              ...
              <Link href={`/${currentLang}/contact/`} className={`
                ...
                ${(isHomePage && !scrolled) 
                    ...
                    : 'bg-[#C02026] text-white shadow-[#C02026]/30 hover:bg-slate-950'}
              `}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-3 custom-scrollbar">
            ...
            <MobileLink href={`/${currentLang}/about-us/`} icon={<LayoutGrid size={20}/>} label={isAr ? 'من نحن' : 'Who we are'} active={isActive(`/${currentLang}/about-us/`)} onClick={closeMenu} />
            ...
            <MobileLink href={`/${currentLang}/blog/`} icon={<Newspaper size={20}/>} label={isAr ? 'المدونة' : 'The Journal'} active={isActive(`/${currentLang}/blog/`)} onClick={closeMenu} />
        </div>
