...
export default function CityCarousel({ lang }) {
  ...
  const scroll = useCallback((direction) => {
    if (scrollRef.current) {
      ...
      // ✅ RTL Logic Fix: في العربي الـ Scroll Left بيشتغل بالقيم السالبة
  ...
  return (
    <section className="py-24 md:py-32 bg-white font-sans overflow-hidden" aria-labelledby="carousel-heading">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div className="space-y-4 text-start">
            <div className="flex items-center gap-3">
               ...
               <span className={`text-[10px] font-black text-[#C02026] uppercase tracking-[0.3em]`}>
                 {isAr ? 'خارطة استثماراتك' : 'Investment Map'}
               </span>
            </div>
            <h2 id="carousel-heading" className={`text-4xl md:text-8xl font-black text-slate-900 italic uppercase tracking-tighter leading-none`}>
              {isAr ? 'أهم الوجهات' : 'Hotspots'}
            </h2>
          </div>
        ...
        >
          {locations.map((city, index) => (
            <Link 
              ...
              className="relative min-w-[320px] md:min-w-[450px] h-[500px] md:h-[650px] rounded-[3.5rem] overflow-hidden group shadow-2xl transition-all duration-700 block snap-center border border-slate-100 bg-slate-200"
            >
              ...
              <div className="absolute inset-0 z-0">
                ...
                <Image 
                  ...
                  fill 
                  priority={index < 2} // ✅ أول منطقتين تحمل صورهم فوراً (LCP)
                  fetchPriority={index < 2 ? "high" : "auto"}
                  sizes="(max-width: 768px) 100vw, 500px"
                  className="object-cover group-hover:scale-110 transition-transform duration-[2s] ease-out opacity-95 group-hover:opacity-100" 
                />
                ) : (
              ...
              {/* --- Content --- */}
              <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-14 z-20 text-start">
                <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="bg-white/10 backdrop-blur-xl border border-white/20 px-6 py-3 rounded-2xl flex items-center gap-3 shadow-2xl">
                    ...
                    <div className="w-12 h-12 rounded-full bg-white text-slate-950 flex items-center justify-center shadow-2xl opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 transition-all duration-500">
                      <ArrowUpRight size={24} className="rtl:rotate-180" aria-hidden="true" />
                    </div>
                  </div>
                </div>
              </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .shadow-premium { box-shadow: 0 40px 100px -20px rgba(0,0,0,0.2); }
      `}} />
    </section>
  );
}