...
import { 
  ...
  Phone, MessageCircle, Wallet, Building2, ArrowUpRight 
} from 'lucide-react';
...
export default function AutoProjectCarousel({ projects, lang, isAr, desktopSlides = 4 }) {
  ...
  return (
    <div className="relative group/swiper px-2 md:px-6" dir="ltr">
      
      {/* 🧭 أزرار التنقل الفاخرة - تظهر عند تحريك الماوس */}
      <div className="absolute top-1/2 -translate-y-1/2 left-1 z-30 opacity-0 group-hover/swiper:opacity-100 transition-all duration-500 hidden lg:block">
        <button className="swiper-button-prev-custom bg-white/90 backdrop-blur-md text-slate-900 p-4 rounded-2xl shadow-2xl hover:bg-[#C02026] hover:text-white transition-all border border-white/20 active:scale-90">
           <ArrowLeft size={24} />
        </button>
      </div>
      <div className="absolute top-1/2 -translate-y-1/2 right-1 z-30 opacity-0 group-hover/swiper:opacity-100 transition-all duration-500 hidden lg:block">
        <button className="swiper-button-next-custom bg-white/90 backdrop-blur-md text-slate-900 p-4 rounded-2xl shadow-2xl hover:bg-[#C02026] hover:text-white transition-all border border-white/20 active:scale-90">
           <ArrowRight size={24} />
        </button>
      </div>
...
const CarouselProjectCard = ({ project, lang, isAr, isPriority }) => {
  ...
  return (
    <div className="group relative bg-white rounded-[2.5rem] p-3.5 border border-slate-100 transition-all duration-500 flex flex-col h-full shadow-[0_10px_30px_-15px_rgba(0,0,0,0.05)] hover:shadow-[0_30px_60px_-20px_rgba(192,32,38,0.15)] hover:-translate-y-2 overflow-hidden">
      ...
      {/* 🖼️ Image Section with Glassmorphism Overlay */}
      ...
      <div className="px-2 flex flex-col flex-grow">
        ...
        <div className="flex items-center gap-2 mb-2">
           ...
           <span className="text-[11px] font-black text-slate-400 uppercase tracking-wide truncate">
             {isAr ? project.developer?.nameAr : project.developer?.nameEn}
           </span>
        </div>

        {/* Project Title */}
        <h3 className="text-xl font-black text-slate-900 mb-2 leading-tight line-clamp-1 group-hover:text-[#C02026] transition-colors">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex flex-col items-center justify-center transition-colors group-hover:bg-white group-hover:border-red-100">
             ...
             <span className="text-[9px] font-bold text-slate-400 uppercase">{isAr ? 'سنوات تقسيط' : 'Years Plan'}</span>
          </div>
          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex flex-col items-center justify-center transition-colors group-hover:bg-white group-hover:border-red-100">
             <span className="text-sm font-black text-slate-900 leading-none mb-1">{project.downPayment || '0'}%</span>
             <span className="text-[9px] font-bold text-slate-400 uppercase">{isAr ? 'مقدم حجز' : 'Down Pay'}</span>
          </div>
        </div>

        {/* 💰 Price Container (Dark UI) */}
        <div className="bg-slate-950 p-4 rounded-[1.8rem] flex items-center justify-between mb-5 mt-auto shadow-xl">
            ...
            <Link href={`/${lang}/projects/${projectSlug}`} className="bg-white/10 hover:bg-[#C02026] text-white w-10 h-10 rounded-xl transition-all flex items-center justify-center group/arrow">
        ...
        <div className="grid grid-cols-2 gap-3 mt-auto">
          <a href={`tel:${CONTACT_INFO.phone}`} className="flex items-center justify-center gap-2 py-4 rounded-2xl bg-slate-100 text-slate-900 hover:bg-slate-950 hover:text-white transition-all font-black text-xs active:scale-95">
            <Phone size={16} /> {isAr ? 'اتصال' : 'Call'}
          </a>
          <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 py-4 rounded-2xl bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366] hover:text-white transition-all font-black text-xs active:scale-95">
