...
// أيقونة تيك توك مخصصة لضمان هوية البراند
...
export default async function Footer({ lang }) {
  ...
  return (
    <footer className="bg-[#080A0D] text-white pt-24 pb-12 border-t border-white/[0.03] relative overflow-hidden" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {/* 🎨 Premium Background Accent - Logical Properties */}
      <div className="absolute top-0 end-0 w-[600px] h-[600px] bg-brand-red/[0.04] rounded-full blur-[120px] pointer-events-none" />

      <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-16 lg:gap-12 border-b border-white/[0.05] pb-20 text-start">
          
          {/* 1. Brand Identity */}
          <div className="lg:col-span-4 space-y-8">
            <Link href={`/${lang}/`} className="group inline-block outline-none" aria-label={data.title}>
            ...
            <div className="flex flex-wrap gap-3">
              {socialLinks.map((item, idx) => (
                <a key={idx} href={item.url} target="_blank" rel="noopener noreferrer" 
                   ...
                   className="w-11 h-11 rounded-xl bg-white/[0.03] border border-white/[0.05] flex items-center justify-center text-slate-300 hover:bg-brand-red hover:text-white hover:-translate-y-1.5 transition-all duration-500">
          ...
          {/* 2. Main Navigation - Semantic & SEO Friendly */}
          <div className="lg:col-span-2">
            ...
            <nav aria-label={isAr ? 'روابط التنقل الأساسية' : 'Main navigation'}>
              <ul className="space-y-4">
                {[
                  { name: isAr ? 'من نحن' : 'About Us', path: 'about-us' },
                  { name: isAr ? 'المطورون' : 'Developers', path: 'developers' },
                  { name: isAr ? 'المدونة' : 'Insights', path: 'blog' },
                  { name: isAr ? 'تواصل معنا' : 'Contact Us', path: 'contact' },
                ].map((link, i) => (
                  <li key={i}>
                    <Link href={`/${lang}/${link.path}/`} className="group flex items-center gap-2 text-[13px] text-gray-300 hover:text-white transition-all font-bold uppercase tracking-tight">
          ...
          {/* 3. Hotspots - Strategic Internal Linking */}
          <div className="lg:col-span-3">
            <h2 className={`text-[11px] font-black mb-10 uppercase flex items-center gap-3 text-slate-300 ${isAr ? 'tracking-wider' : 'tracking-[0.3em]'}`}>
              {isAr ? 'أهم المناطق' : 'Hotspots'}
              <div className="h-px flex-1 bg-gradient-to-r from-brand-red to-transparent opacity-30"></div>
            </h2>
            <nav aria-label={isAr ? 'روابط أهم المناطق' : 'Hotspots navigation'}>
          ...
          <div className="lg:col-span-3 space-y-10">
            ...
            <div className="space-y-6">
              <a href={data.mapLink} target="_blank" rel="noopener noreferrer" 
                 className="flex items-start gap-4 group p-2 -m-2">
              ...
              <a href={`https://wa.me/${data.whatsapp}`} target="_blank" rel="noopener noreferrer" 
                 className="flex items-center gap-4 group p-2 -m-2">
        ...
        <div className="py-10 flex flex-col md:flex-row justify-between items-center gap-8 border-t border-white/[0.02]">
          ...
          <div className="flex flex-wrap justify-center gap-4 md:gap-8 items-center">
            {[
              { id: 'Privacy', ar: 'الخصوصية', path: 'privacy' },
              { id: 'Terms', ar: 'الشروط', path: 'terms' },
              { id: 'Sitemap', ar: 'خريطة الموقع', path: 'sitemap' }
            ].map((item) => (
              <Link key={item.id} href={`/${lang}/${item.path}/`} className="text-gray-400 hover:text-brand-red text-[10px] uppercase tracking-[0.2em] font-black transition-colors px-2 py-1">
                {isAr ? item.ar : item.id}
              </Link>
            ))}
          </div>
        </div>
