...
// ✅ 3. الميتا داتا للأرشفة (SEO)
export async function generateMetadata({ params }: { params: Promise<{ slug: string, lang: string }> }) {
  const { slug, lang } = await params;
  const isAr = lang === 'ar';
  const district = await client.fetch(`*[_type == "district" && slug.current == $slug && !(_id in path("drafts.**"))][0]{nameAr, nameEn}`, { slug });
  
  if (!district) return { title: 'District Not Found' };
  
  const name = isAr ? district.nameAr : district.nameEn;
  return {
    title: `${name} | ${isAr ? 'أفضل العقارات' : 'Prime Real Estate'} | ${CONTACT_INFO.siteNameEn}`,
    description: isAr 
    ...
    alternates: {
      canonical: `${CONTACT_INFO.domain}/${lang}/districts/${slug}`,
    }
  };
}

// ✅ نظام التنسيق البصري للمحتوى
                ...
                <div className="bg-[#111827] rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl border-b-8 border-[#C02026]">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="space-y-4">
                            <a href={`https://wa.me/${CONTACT_INFO.whatsapp.replace(/\D/g,'')}`} className="flex items-center justify-center gap-3 w-full py-5 bg-[#25D366] hover:bg-[#1eb954] text-white rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl shadow-green-500/10">
