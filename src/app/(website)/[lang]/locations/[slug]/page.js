...
// ✅ 1. إضافة دالة توليد المسارات الثابتة (SSG)
export async function generateStaticParams() {
  const query = `*[_type == "location" && defined(slug.current)]{ "slug": slug.current }`;
  const locations = await client.fetch(query);
  
  // توليد المسارات لكل منطقة باللغتين العربية والإنجليزية
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC]" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 relative z-30">
        ...
        {/* 2. PREMIUM DISTRICTS NAV (تصميم الأحياء الدائري الفخم) */}
        ...
        {/* 3. MAIN CONTENT LAYOUT (8 Columns Main | 4 Columns Sidebar) */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          
          {/* A. CONTENT SIDE (Projects & Guide) */}
          ...
          {/* B. STICKY SIDEBAR CTA (صندوق التواصل الجانبي) */}
          <aside className="lg:col-span-4">
            <div className="lg:sticky lg:top-28 space-y-8">
                <div className="bg-[#111827] text-white p-10 rounded-[3rem] shadow-2xl border-b-[12px] border-[#C02026] relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-slate-400 text-sm mb-10 font-medium leading-relaxed">
                            {isAr 
                                ? `نحن نساعدك في مقارنة الأسعار واختيار أفضل نظام سداد في المنطقة مجاناً وبدون أي عمولات.` 
                ...
                {/* كارت أحياء سريعة (Internal Linking) */}
